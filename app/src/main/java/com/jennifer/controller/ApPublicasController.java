...
public class ApPublicasController extends AppCompatActivity {
    ...
    public class CompetitorLoad extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            try {
                OkHttpClient client = new OkHttpClient();
                Request request = new Request.Builder().url("http://excellentprogrammers.esy.es/Script/apPublicas/apPublic.php?TFX=GLC&MTCH="+(this.matchId)).build();
                Response response = client.newCall(request).execute();
                String result = response.body().string();
    ...
    private void procesaDatos(int compElegido, int idBet, double montoApuesta){
        int idUsuario = new Shared().getShared(ApPublicasController.this,"ID");
        int idApuesta = idBet;
